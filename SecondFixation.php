...
foreach ($filelist as $filename) {
  ...
  $second_fixation = fgetcsv($file);

  $pathinfo = pathinfo($filename);
  $exploded_filename = explode('_', $pathinfo['filename']);
  $output_data[] = [$exploded_filename[0], $exploded_filename[1], $second_fixation[0], $second_fixation[1]];
}

$output_file = $target_dir . '/' . 'SecondFixation.csv';
touch($output_file);
